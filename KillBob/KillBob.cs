using System;
using UnityEngine;
using System.Collections.Generic;
using KSP.UI.Screens;

namespace KillBob
{
    [KSPAddon(KSPAddon.Startup.Flight, false)]
    public class KillWheelBob : MonoBehaviour
    {

        public bool KillAutoBoost = true, KillAutoStruts = false;
        public ApplicationLauncherButton appButton;
        public void Awake()
        {
           
           // GameEvents.onGUIApplicationLauncherReady.Add(this.app_launcher_ready);
           // GameEvents.onGUIApplicationLauncherDestroyed.Add(this.app_launcher_destroyed);

        }
        public void OnDestroy()
        {
            //app_launcher_destroyed();
        }

        public void app_launcher_ready()
        {
            GameEvents.onGUIApplicationLauncherReady.Remove(this.app_launcher_ready);
            Texture2D tex = new Texture2D(2, 2);
            // A small 64x64 Unity logo encoded into a PNG.
            byte[] pngBytes = new byte[] {
            ,
             };
            // Load data into the texture.
            tex.LoadImage(pngBytes);
            appButton = ApplicationLauncher.Instance.AddModApplication(onToolBarClick, onToolBarClick, onToolbarHover, onToolbarHoverOut, null, null, ApplicationLauncher.AppScenes.FLIGHT, tex);
        }
        public void app_launcher_destroyed()
        {
            if (appButton == null) return;
            ApplicationLauncher.Instance.RemoveModApplication(appButton);
            appButton = null;
        }


        private bool started = false;
        public void Start()
        {
            if (started) return;

                //updateAutoBoost();
                started = true;
        }

        public void onToolBarClick()
        {
            showWindow();
        }
        public void onToolbarHover()
        {
        }
        public void onToolbarHoverOut()
        {

        }

        public void showWindow()
        {
                PopupDialog.SpawnPopupDialog(new Vector2(0.5f, 0.5f),
                new Vector2(0.5f, 0.5f),
                new MultiOptionDialog("",
                            "Wheel Mods", "",
                            HighLogic.UISkin,
                            
                            new Rect(0.5f, 0.5f, 150f, 60f),
                            new DialogGUIFlexibleSpace(),
                            new DialogGUIVerticalLayout(
                                new DialogGUIFlexibleSpace(),
                                new DialogGUIToggleButton(KillAutoBoost, "Kill Autoboosting Suspension", 
                                    delegate {
                                        KillAutoBoost = !KillAutoBoost;
                                        updateAutoBoost();
                                    }, 140.0f, 30.0f),
                                new DialogGUIToggleButton(KillAutoStruts, "Kill Locked Autostruts",
                                    delegate {
                                        KillAutoStruts = !KillAutoStruts;
                                        updateAutoStruts();
                                    }, 140.0f, 30.0f),
                                new DialogGUIButton("Close", () => { }, 140.0f, 30.0f, true)
                                )),
                        false,
                        HighLogic.UISkin, false);
                
            }

        public void FixedUpdate()
        {
            //if (KillAutoBoost) updateAutoBoost();
        }
        public void Update()
        {
            //if (KillAutoBoost) updateAutoBoost();
            //if (KillAutoStruts) updateAutoStruts();

        }

            public void updateAutoBoost()
        {
            List<ModuleWheels.ModuleWheelSuspension> myParts = FlightGlobals.ActiveVessel.FindPartModulesImplementing<ModuleWheels.ModuleWheelSuspension>();
            foreach (ModuleWheels.ModuleWheelSuspension mws in myParts)
            {
                Debug.Log(string.Format("{0}, {1}", mws.damperTweakable, mws.springTweakable));
                mws.useAutoBoost = false;
            }   


            /*
            List<ModuleWheels.ModuleWheelSuspension> mws = FlightGlobals.ActiveVessel.FindPartModulesImplementing<ModuleWheels.ModuleWheelSuspension>();
            for (int i=0; i< mws.Count; i++)
            {
                ModuleWheels.ModuleWheelSuspension s = mws[i];
                

            }
            */
            /*
            List<Part> myParts = FlightGlobals.ActiveVessel.FindPartModulesImplementing<Part>();
            Debug.Log(string.Format("parts found: {0}", myParts.Count));
            foreach (Part p in myParts)
            { 
                if (KillAutoBoost)
                {
                    ModuleWheels.ModuleWheelSuspension mws = p.FindModuleImplementing<ModuleWheels.ModuleWheelSuspension>();
                    if (mws != null)
                    {
                        Debug.Log(string.Format("mws found: {0}", mws.GetInstanceID()));
                        p.RemoveModule(mws);
                        suspension s = new suspension();
                        s.useAutoBoost = false;
                        p.Modules.Add(s);
                    }
                }
                else // restore original suspension
                {
                    suspension mws = p.FindModuleImplementing<suspension>();
                    if (mws != null)
                    {
                        p.RemoveModule(mws);
                        p.Modules.Add(new ModuleWheels.ModuleWheelSuspension());
                    }
                }

            }
            */
        }

        public void updateAutoStruts()
        {
            List<Part> myListParts = FlightGlobals.ActiveVessel.FindPartModulesImplementing<Part>();
            foreach (Part ps in myListParts)
                if (ps.FindModuleImplementing<ModuleWheelBase>() != null)
                {
                    if (KillAutoStruts)
                    {
                        ps.autoStrutMode = Part.AutoStrutMode.Off;
                        ps.ReleaseAutoStruts();
                    }
                    else
                    {
                        ps.autoStrutMode = Part.AutoStrutMode.Heaviest;
                        ps.UpdateAutoStrut();
                    }
                }

        }

        public void OnVesselChange(Vessel v)
        {

        }

        
    }

   
}
